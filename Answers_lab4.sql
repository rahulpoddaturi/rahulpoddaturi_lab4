...
select CUS_ID from `order` where ORD_AMOUNT >= 3000) group by CUS_GENDER ;
 ...
 select product.PRO_NAME as `Name`, 
        temp.ORD_ID as ID,
		temp.ORD_AMOUNT as Amount,
		temp.ORD_DATE as `Order Date`
        from (
 select `order`.ORD_ID ,
		`order`.ORD_AMOUNT ,
		`order`.ORD_DATE ,
        supplier_pricing.PRO_ID from `order` 
 join supplier_pricing
 on `order`.PRICING_ID = supplier_pricing.PRICING_ID
and CUS_ID = 2) as temp
join product 
on product.PRO_ID = temp.PRO_ID;
